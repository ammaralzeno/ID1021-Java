...
public class BreadthTree implements Iterable<Integer> {
    ...
    public Iterator<Integer> iterator() {
        return new BreadthFirstTreeIterator();
    }

    private class BreadthFirstTreeIterator implements Iterator<Integer> {
        private Node next;
        private Queue<Node> queue = new LinkedList<>(); // Queue to keep track of nodes

        public BreadthFirstTreeIterator() {
            if(root != null) {
                queue.add(root); // Add root to start traversal from root
                next = queue.poll(); // Initialize next
            }
        }
        ...
        public Integer next() {
            ...
            // Enqueue left child if it is not null
            if (curr.left != null) {
                queue.add(curr.left);
            }

            // Enqueue right child if it is not null
            if (curr.right != null) {
                queue.add(curr.right);
            }

            // Move to next node in the queue (null if queue is empty)
            next = queue.poll();
